...
public class HttpURLConnHelper {
	/**
	 * 作用：实现网络访问文件，将获取到数据储存2015/12/10在文件流中
	 * 
	 * @paramurl
	 *            ：访问网络的url地址
	 * @return inputstream
	 ...
	 *            qq=32432432	 *            为了防止传中文参数时出现编码问题。采用URLEncoder.encode()对含中文的字符串进行编码处理。
	 *            服务器端会自动对进行过编码的字符串进行decode()解码。
	 * @return byte[]
	 */
	public static byte[] doPostSubmit(String url, String params,String uid) {
		BufferedOutputStream bos = null;
		BufferedInputStream bis = null;
		HttpURLConnection httpConn = null;
		try {
